...
export default function BiernikCard({
  currentQuestion,
  onAnswerChange,
  onCheckAnswer,
  onNextQuestion,
  ...
  const rule = accusativeRules[currentQuestion.sentence.gender];

  // Check if the accusative is the same as nominative
  const sameAsNominative =
    currentQuestion.sentence.nominative === currentQuestion.sentence.answer;

  return (
    <div className="bg-slate-800/70 backdrop-blur-xl rounded-3xl border border-slate-700/50 shadow-2xl overflow-hidden">
      {/* Noun info header */}
      <div className="bg-gradient-to-r from-rose-600/20 via-pink-500/15 to-rose-600/20 px-8 py-5 border-b border-slate-700/50">
      ...
      <div className="p-8">
        <div className="text-slate-400 text-sm uppercase tracking-wider mb-4 text-center">
          Fill in the Accusative (Biernik)
        </div>
          ...
          <input
            ...
            className={`w-40 md:w-52 px-4 py-2 text-center bg-slate-900/50 border-2 border-dashed rounded-xl outline-none transition-all duration-300 placeholder:text-slate-600 ${
              currentQuestion.isCorrect === null
                ? "border-slate-500 focus:border-rose-500/50 text-white"
            ...
            <div className="mt-4 pt-4 border-t border-slate-700/50">
              ...
              {sameAsNominative && (
                <div className="text-xs text-slate-500 mt-2 italic">
                  (The form stays the same: {currentQuestion.sentence.nominative} → {correctAnswer})
                </div>
              )}
              {!sameAsNominative && (
                <div className="text-xs text-slate-500 mt-2 italic">
          ...
          <div className="text-center mb-6">
            ...
            {showHint && (
              <div className="mt-3 p-4 bg-slate-900/50 rounded-xl border border-slate-700 inline-block">
                ...
                <div className="text-amber-400 text-sm mb-2">{rule}</div>
                {sameAsNominative ? (
                  <div className="text-slate-500 text-xs">
                    This word stays the same in accusative!
                  </div>
                ) : (
                  <div className="text-slate-500 text-xs">
                    First letter:{" "}
