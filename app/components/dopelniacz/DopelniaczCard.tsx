...
import {
  type DopelniaczExercise,
  genderLabels,
  genitiveRules,
} from "../../data/dopelniacz";
...
export default function DopelniaczCard({
  currentQuestion,
  onAnswerChange,
  onCheckAnswer,
  onNextQuestion,
  ...
  const rule = genitiveRules[currentQuestion.sentence.gender];

  // Check if the genitive is the same as nominative
  ...
  return (
    <div className="bg-slate-800/70 backdrop-blur-xl rounded-3xl border border-slate-700/50 shadow-2xl overflow-hidden">
      {/* Noun info header */}
      <div className="bg-gradient-to-r from-amber-600/20 via-orange-500/15 to-amber-600/20 px-8 py-5 border-b border-slate-700/50">
      ...
      <div className="p-8">
        <div className="text-slate-400 text-sm uppercase tracking-wider mb-4 text-center">
          Fill in the Genitive (Dopełniacz)
        </div>
          ...
          <input
            ...
            className={`w-40 md:w-52 px-4 py-2 text-center bg-slate-900/50 border-2 border-dashed rounded-xl outline-none transition-all duration-300 placeholder:text-slate-600 ${
              currentQuestion.isCorrect === null
                ? "border-slate-500 focus:border-amber-500/50 text-white"
              ...
              <div className="mt-3 p-4 bg-slate-900/50 rounded-xl border border-slate-700 inline-block">
                ...
                {sameAsNominative ? (
                  <div className="text-slate-500 text-xs">
                    This word stays the same in genitive!
                  </div>
                ) : (
                  <div className="text-slate-500 text-xs">
                    First letter:{" "}
